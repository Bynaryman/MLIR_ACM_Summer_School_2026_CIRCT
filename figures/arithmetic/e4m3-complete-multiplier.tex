...
\begin{tikzpicture}
  ...
  \draw[wire=signcolor] (sign.south) -- ++(0,-3.70)
    -| ($(select.north)+(-3.75,0)$);
  \node[signal=signcolor, anchor=west] at (-4.37,-0.72) {result sign};

  \draw[control=lightneutral] (decode.east) -- ++(0.48,0)
    |- ($(select.north)+(3.75,0)$);
  \node[signal=lightneutral, rotate=90] at (5.98,-0.25) {class flags};

...
`lib/LowerE4M3FNToComb.cpp`. The input deliberately uses `f8E4M3FN` arguments
and contains only one `arith.mulf`. Supplied dialect-conversion scaffolding
changes each `f8E4M3FN` SSA value to its `i8` wire encoding, including the
function signature and return. The student-written pattern builds the `comb`
datapath from those converted operands.

The passes run in this order:

```bash
circt-tutorial-opt examples/ex7_e4m3fn_mul.mlir \
  --lower-e4m3fn-to-comb \
  --tutorial-func-to-hw
```

`--tutorial-func-to-hw` runs second because it only converts the structural
boundary; it does not reinterpret numerical types as integers.
